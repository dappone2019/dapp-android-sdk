package com.blockchain.dappbirds.opensdk.ont.ontio.smartcontract.neovm;
...
import com.blockchain.dappbirds.opensdk.ont.ontio.smartcontract.neovm.abi.AbiInfo;
...
import com.blockchain.dappbirds.opensdk.ont.ontio.core.asset.State;
...
public class Oep4 {
    ...
    private String oep4abi = "{\"hash\":\"0xcf409996f7abbf8afd659a298c26cd1239e0c19e\",\"entrypoint\":\"Main\",\"functions\":[" +
            ...
            "{\"name\":\"PayOrder\",\"parameters\":[{\"name\":\"fromAcct\",\"type\":\"ByteArray\"},{\"name\":\"orderNo\",\"type\":\"String\"},{\"name\":\"amount\",\"type\":\"Integer\"}],\"returntype\":\"Boolean\"}," +
    ...
    public String sendTransfer(Account acct, String recvAddr, long amount, Account payerAcct, long gaslimit, long gasprice, String order_no) throws Exception {
        return (String) sendTransfer(acct, recvAddr, amount, payerAcct, gaslimit, gasprice, false, order_no);
    }

    public long sendTransferPreExec(Account acct, String recvAddr, long amount, String order_no) throws Exception {
        return (long) sendTransfer(acct, recvAddr, amount, acct, 0, 0, true, order_no);
    }

    private Object sendTransfer(Account acct, String recvAddr, long amount, Account payerAcct, long gaslimit, long gasprice, boolean preExec, String order_no) throws Exception {
        if (contractAddress == null) {
            throw new SDKException(ErrorCode.NullCodeHash);
        }
        ...
        AbiInfo abiinfo = JSON.parseObject(oep4abi, AbiInfo.class);
        AbiFunction func = null;
        if (order_no == null) {
            func = abiinfo.getFunction("Transfer");
            func.name = "transfer";
            func.setParamsValue(Address.decodeBase58(sendAddr).toArray(), Address.decodeBase58(recvAddr).toArray(), amount);
        } else {
            func = abiinfo.getFunction("PayOrder");
            func.name = "payOrder";
            func.setParamsValue(Address.decodeBase58(sendAddr).toArray(), order_no, amount);
        }
        if (preExec) {
            byte[] params = BuildParams.serializeAbiFunction(func);
