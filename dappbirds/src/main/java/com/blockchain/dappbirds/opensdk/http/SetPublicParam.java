...
public class SetPublicParam {
    ...
    public HashMap<String, String> setSomeParam(Context context) {
        ...
        if (context == null) {
            ...
            try {
                map.put(key_app_id, AbSharedUtil.getAppID(context));
                map.put(key_openid, AbSharedUtil.getOpenID(context));
                map.put(key_chain_type, AbSharedUtil.getChaintype(context));
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
