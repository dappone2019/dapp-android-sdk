...
import static com.blockchain.dappbirds.opensdk.ont.ontio.crypto.bip32.BigIntegerUtils.parse256;
import static com.blockchain.dappbirds.opensdk.ont.ontio.crypto.bip32.BigIntegerUtils.ser256;
...
public class ExtendedPrivateKey implements
    ...
    public static ExtendedPrivateKey fromSeed(final byte[] seed, final Network network) {
        final byte[] I = HmacSha512.hmacSha512(BITCOIN_SEED, seed);

        final byte[] Il = ByteArrayWriter.head32(I);
        final byte[] Ir = ByteArrayWriter.tail32(I);
    ...
    public static ExtendedPrivateKey fromSeed(final byte[] seed,byte[] byteKey, final Network network) {
        final byte[] I = HmacSha512.hmacSha512(byteKey, seed);
    ...
    private static byte[] getBytes(final String seed) {
        return CheckedExceptionToRuntime.toRuntime(new CheckedExceptionToRuntime.Func<byte[]>() {
            @Override
    ...
    public ExtendedPrivateKey cKDpriv(final int index) {
        ...
        final byte[] I = HmacSha512.hmacSha512(hdKey.getChainCode(), data);
        ...
        final BigInteger ki = parse256_Il.add(parse256(key)).mod(Secp256r1SC.n());
