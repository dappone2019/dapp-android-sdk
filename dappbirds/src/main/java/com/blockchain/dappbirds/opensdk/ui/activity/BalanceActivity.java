...
public class BalanceActivity extends Activity {
                ...
                switch (keyType) {
                    ...
                    case 3:
                        try {
                            ...
                            Map map = WalletQR.exportAccountQRCode(scrypt, account);
                            String priKeyFromQrCode = WalletQR.getPriKeyFromQrCode(JSON.toJSONString(map), password);
                            if (!Kits.Empty.check(priKeyFromQrCode) && !Kits.Empty.check(AbSharedUtil.getMnemonic(context))) {
                                dismissPorcess();
                                startActivity(createIntent(AbSharedUtil.getMnemonic(context), 3));
                            } else {
                                Toast.makeText(context, "没有助记词", Toast.LENGTH_SHORT).show();
                            }
                        } catch (Exception e) {
                            if (DappBirdsSdk.isEnableLog()) {
                                Log.e(TAG, e.getMessage());
    ...
    private void setListener() {
        ...
        mMenuIcon.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                popupWindow.showAsDropDown(v, -15, 0);
            }
        });
    ...
    private void initView() {
        ...
        String string = AbSharedUtil.getCreateType(this);
        list.clear();
