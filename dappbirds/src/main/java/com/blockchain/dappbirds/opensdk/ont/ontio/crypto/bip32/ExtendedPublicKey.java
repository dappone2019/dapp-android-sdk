...
public final class ExtendedPublicKey implements
    ...
    public ExtendedPublicKey cKDpub(final int index) {
        ...
        final byte[] I = HmacSha512.hmacSha512(parent.getChainCode(), data);
        ...
        final ECPoint ki = Secp256r1SC.gMultiplyAndAddPoint(parse256_Il, kPar);

        if (parse256_Il.compareTo(Secp256r1SC.n()) >= 0 || ki.isInfinity()) {
            return cKDpub(index + 1);
        }

        final byte[] key = Secp256r1SC.pointSerP(ki);
    ...
    public String p2shAddress() {
        ...
        script[1] = (byte) 20;
        Hash160.hash160into(script, 2, hdKey.getKey());
