...
package com.blockchain.dappbirds.opensdk.ont.ontio.sdk.manager;

import android.content.SharedPreferences;

import com.blockchain.dappbirds.opensdk.ont.ontio.crypto.Curve;
import com.blockchain.dappbirds.opensdk.ont.ontio.crypto.ECC;
import com.blockchain.dappbirds.opensdk.ont.ontio.crypto.KeyType;
...
import com.blockchain.dappbirds.opensdk.ont.ontio.sdk.info.AccountInfo;
import com.blockchain.dappbirds.opensdk.ont.ontio.sdk.info.IdentityInfo;
...
public WalletMgr(String path, SignatureScheme scheme) throws Exception {
    ...
    public Identity importIdentity(String label,String encryptedPrikey, String password,byte[] salt, String address) throws Exception {
        String prikey = com.blockchain.dappbirds.opensdk.ont.ontio.account.Account.getGcmDecodedPrivateKey(encryptedPrikey, password, address,salt, walletFile.getScrypt().getN(), scheme);
    ...
    public IdentityInfo getIdentityInfo(String ontid, String password,byte[] salt) throws Exception {
        com.blockchain.dappbirds.opensdk.ont.ontio.account.Account acct = getAccountByAddressOrOntId(ontid, password,salt);
    ...
    private IdentityInfo createIdentity(String label,String password,byte[] salt, byte[] prikey) throws Exception {
        com.blockchain.dappbirds.opensdk.ont.ontio.account.Account acct = createAccount(label,password,salt, prikey, false);
        IdentityInfo info = new IdentityInfo();
        info.ontid = Common.didont + Address.addressFromPubKey(acct.serializePublicKey()).toBase58();
        info.pubkey = Helper.toHexString(acct.serializePublicKey());
        info.setPrikey(Helper.toHexString(acct.serializePrivateKey()));
    ...
    private AccountInfo createAccountInfo(String label,String password,byte[] salt, byte[] prikey) throws Exception {
        com.blockchain.dappbirds.opensdk.ont.ontio.account.Account acct = createAccount(label,password,salt, prikey, true);
    ...
    public String privateKeyToWif(String privateKey) throws Exception {
        com.blockchain.dappbirds.opensdk.ont.ontio.account.Account act = new com.blockchain.dappbirds.opensdk.ont.ontio.account.Account(Helper.hexToBytes(privateKey), scheme);
        return act.exportWif();
    }

    public com.blockchain.dappbirds.opensdk.ont.ontio.account.Account getAccount(String addressOrOntId, String password, byte[] salt) throws Exception {
    ...
    public AccountInfo getAccountInfo(String addressOrOntId, String password,byte[] salt) throws Exception {
        AccountInfo info = new AccountInfo();
        com.blockchain.dappbirds.opensdk.ont.ontio.account.Account acc = getAccountByAddressOrOntId(addressOrOntId, password,salt);
    ...
    private com.blockchain.dappbirds.opensdk.ont.ontio.account.Account createAccount(String label, String password, byte[] salt, byte[] privateKey, boolean accountFlag) throws Exception {
        com.blockchain.dappbirds.opensdk.ont.ontio.account.Account account = new com.blockchain.dappbirds.opensdk.ont.ontio.account.Account(privateKey, scheme);
        Account acct;
        switch (scheme) {
    ...
    private com.blockchain.dappbirds.opensdk.ont.ontio.account.Account getAccountByAddressOrOntId(String addressOrOntId, String password, byte[] salt) throws Exception {
        try {
            if(addressOrOntId.startsWith(Common.didont)){
                for (Identity e : walletInMem.getIdentities()) {
                    if (e.ontid.equals(addressOrOntId)) {
                        String addr = e.ontid.replace(Common.didont, "");
                        String prikey = com.blockchain.dappbirds.opensdk.ont.ontio.account.Account.getGcmDecodedPrivateKey(e.controls.get(0).key, password, addr, salt,walletFile.getScrypt().getN(), scheme);
                        return new com.blockchain.dappbirds.opensdk.ont.ontio.account.Account(Helper.hexToBytes(prikey), scheme);
                    }
                }
            }else{
                for (Account e : walletInMem.getAccounts()) {
                    if (e.address.equals(addressOrOntId)) {
                        String prikey = com.blockchain.dappbirds.opensdk.ont.ontio.account.Account.getGcmDecodedPrivateKey(e.key, password, e.address, salt,walletFile.getScrypt().getN(), scheme);
                        return new com.blockchain.dappbirds.opensdk.ont.ontio.account.Account(Helper.hexToBytes(prikey), scheme);
                    }
                }
            }
