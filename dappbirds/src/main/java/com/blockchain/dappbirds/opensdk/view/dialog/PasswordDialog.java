                ...
                if (payCallBack != null) {
                    ...
                    if (edPassword != null) {
                        if (!Kits.Empty.check(edPassword.getText().toString()) && freepassword.isChecked()) {
                            AbSharedUtil.setPassword(context, edPassword.getText().toString());
                        } else {
                            AbSharedUtil.setPassword(context, "");
                        }
                    }
                    AbSharedUtil.setPassSuccessTime(context, System.currentTimeMillis() + "");
                }
            }
            dismissPorcess();
            dismiss();
                    ...
                    new Thread(new Runnable() {
                        ...
                        public void run() {
                            try {
                                ...
                                ontSdk.openWalletFile(activity.getSharedPreferences(Contants.SHARED_PATH + AbSharedUtil.getBaseKey(context), Context.MODE_PRIVATE));
