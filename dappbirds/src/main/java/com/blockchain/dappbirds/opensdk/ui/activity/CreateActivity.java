...
public class CreateActivity extends Activity {
    ...
    private void createSuccess() {
        ...
        NetApi.walletStore(hashMap, new JsonCallback<BaseModel>() {
            ...
            public void onResponse(BaseModel response, int id) {
                if (response.code == 0) {
                    is_success = true;
                    AbSharedUtil.setAddress(context, currentAccout.address);
                    handler.sendEmptyMessage(1);
                    if (!Kits.Empty.check(codesStr)) {
                        AbSharedUtil.setMnemonic(context, codesStr);
                    }
                } else {
                    is_success = false;
    ...
    private void createByMnemonic() {
        try {
            ...
            AbSharedUtil.setMnemonic(context, inputContent.getText().toString());
            ...
            AbSharedUtil.setCreateType(this, "3");
        } catch (Exception e) {
            createError(e);
        }
    }
    ...
    private void createByKeyStore() {
        try {
            ...
            walletMgr.writeWallet();
            AbSharedUtil.setCreateType(this, "2");
        } catch (Exception e) {
            createError(e);
        }
    }
    ...
    private void createByPriateKey() {
        try {
            ...
            walletMgr.writeWallet();
            AbSharedUtil.setCreateType(this, "1");
        } catch (Exception e) {
            createError(e);
        }
    }
    ...
    private void createByNomal() {
        try {
            ...
            walletMgr.createAccountFromPriKey(edName.getText().toString(), edPassword.getText().toString(), Helper.toHexString(prikey));
            walletMgr.writeWallet();
            AbSharedUtil.setCreateType(this, "0");
        } catch (Exception e) {
            createError(e);
        }
    }
    ...
    private void initView() {
        ...
        try {
            ...
            ontSdk.openWalletFile(getSharedPreferences(Contants.SHARED_PATH + AbSharedUtil.getBaseKey(context), Context.MODE_PRIVATE));
